...
**Да, с оговорками.** Seed использует `upsert` — существующие данные не дублируются. Но ряд полей **перезаписывается**:

- **Пользователи** — `update`-блок сбрасывает `firstName`, `lastName`, `role`. Если через API поменяли роль пользователя `admin@test.com` — seed вернёт обратно
- **Промпты** — находит активный промпт по типу и перезаписывает его `content`. Отредактировал промпт через UI (`/prompts`) → запустил seed → изменения потеряны
- **Knowledge documents** с `id` вида `seed-knowledge-*` — контент перезаписывается. Документы добавленные через API (другой `id`) не затрагиваются
