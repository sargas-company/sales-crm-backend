  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  private buildCallReminderEmbed(
    ...
    const timeLabel = is60min ? '60 minutes' : '10 minutes';

    const fields = [
      { name: 'Reminder', value: `⏰ ${timeLabel} before`, inline: true },
      { name: 'Kyiv time', value: p.kyivDateTime, inline: true },
      { name: 'Meeting', value: p.meetingUrl ?? 'No link', inline: false },
    ];
